function save_2d_field_as_vtk(nodes, elements, field, field_location_type,  output_filename, title)
%
...
vtk_write_field(fid, title, number_nodes, number_elements, element_order, xyz, element_node, field', field_location_type);
...
function vtk_write_field(fid, title, number_nodes, number_elements, element_order, xyz, element_node, field, field_location_type)
...
field = full(field);
if (strcmpi(field_location_type, 'node'))
    fprintf(fid, '\n');
...
elseif (strcmpi(field_location_type, 'centroid'))
    fprintf(fid, '\n');
